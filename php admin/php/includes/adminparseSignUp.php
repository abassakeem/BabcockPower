...
if (isset($_POST['submit'])){
    ...
    $fields_to_check_lenght= array('firstname' => 3,'lastname' => 3,'username' => 10,'email_address' => 5,'password' => 8,'phone_number' => 11,'address' => 10,'zip_code' => 5);
    ...
    $role="admin";
        ...
        if(checkDuplicateInput("userstable","username",$username, $db)){
            $result = flashMessage("Username is being used by another customer");
        }
    ...
    elseif (empty($errors)){
        ...
        try{
            $sqlInsert = "INSERT INTO userstable (users_id,firstname,lastname,roles,username,date_of_birth,email_address,passwords,phone_number,addresss,states,zip_code,created_at) VALUES (:users_id,:firstname,:lastname,:roles,:username,:dob,:email_address,:passwords,:phone_number,:addresss,:states,:zip_code, now())";

        $statement= $db->prepare($sqlInsert);
        $statement->execute(array(':users_id'=>$users_id,':firstname' => $firstname,':lastname'=> $lastname,':username' => $username,':dob' => $dob, ':email_address' => $email_address,':passwords' => $hashed_password,':roles'=> $role,':phone_number' => $phone_number,':addresss' => $addresss,':states' => $states,':zip_code' => $zip_code));
