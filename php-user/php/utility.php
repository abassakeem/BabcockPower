...
//$required_fields = array('firstname','lastname','meterid','date_of_birth','email_address','password','phone_number','address','address','state','zip_code');
//loop through the required fields and populate the form error array
   ...
   // $result .= " style='color:red;'>";
    ...
    function flashMessage($message, $passOrFail ="Fail"){
        if($passOrFail == "pass" || $passOrFail == "Pass"){
            $data ="<p style='padding:20px; color:magenta';>{$message}<br></p>";

        }elseif($passOrFail == "fail" || $passOrFail == "Fail"){
            $data = "<p style='color:red;'>{$message}<br>";
    ...
    function toastMessage($message){
            ...
            $data =" <div aria-live ='polite' aria-atomic='true' class='row justify-content-center align-items-center w-100'>
...
function guard(){
        ...
        if((isset($_SESSION['fingerprint']) && $_SESSION['fingerprint'] != $fingerprint)){
            $isValid = false;
            signout();
        }elseif((isset($_SESSION['last_active']) && (time() - $_SESSION['last_active']) > $inactive) && $_SESSION['meterid']){
            $isValid = false;
            signout();
