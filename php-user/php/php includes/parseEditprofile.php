...
if((isset($_SESSION['id']) || isset($_GET['user_identity'])) && !isset($_POST['updateProfileButton'])){
  ...
  $user_pic ="../abass svg for project/".$firstname.".jpg";
...
if($statement->rowCount() == 1 || uploadProfilePicture($meterid)){
  $result = '<script> window.location.href = "myprofile.php"</script>';
  $_SESSION['result'] = toastMessage('Changes successfully updated');
  ...
  $_SESSION['result']= toastMessage('Nothing happened, you have not made any changes');
}
  ...
  $_SESSION['result']= toastMessage('An error occured in : ' . $ex->getMessage());
    }
  }
 else{
   if(count($errors) == 1){
    ...
    $_SESSION['result'] = toastMessage('There was 1 error in the form<br>');
   }else{
    
    $result = '<script> window.location.href = "myprofile.php"</script>';
  
    $_SESSION['result'] = toastMessage('there were ' . count($errors) . ' errors in the form');
   }
 }
}
